...
| `data/follows.json` | Follow-provenance sidecar (like `credits.json`): per-artist `followed_at`, the song captured at the follow (`trigger_track` + confidence), and `seed_ties` — co-artists on that track who are in the collection, the earned edges a seeded artist wires into the graph through. Written by `harvest_merge.py`; the raw event log is `data/harvests/follow-events.jsonl`. |
| `harvest_merge.py` | Stdlib-only fold (sibling to `streaming_merge.py`): reads the follow-event log and folds it in. A follow is a *deliberate act*, so it may auto-seed — new artists append to the reservoir untagged (`source: spotify-follow`); a follow of an owned artist records provenance only; a follow of a **discarded** artist and any **unfollow** are recorded but flagged for review, never auto-applied. Idempotent via an event ledger. Emits a `--summary` the automation reads to decide whether to arm auto-merge. |
| `curator_lib.py` | Shared stdlib primitives — chiefly `alnum()`, the Phase-2 dedup key (accent-folding; every layer joins to the roster through it), plus `index_by_alnum` / `collisions_by_alnum`. Imported by the merge tools and `validate.py`. |
...
| `obsidian_driver.py` | Stdlib-only driver that renders `data/` into the vault (each artist note → one hub wikilink in a two-tier tree: 13 color-coded top-level categories, second-order subcategory hubs beneath). Folds in the streaming layer (`rotation` class + evidence + histogram, a `Rotation` MOC, a `rotation` preset) and the follow layer (`followed_at` + a **Followed:** provenance line + `source-follow` tag/preset; seed-tie edges from the trigger song). Its `build_personnel_edges` re-resolves personnel against the live roster by `alnum`, so a newly-seeded artist wires in via existing credits without re-running the research. Sibling to `validate.py`. |
