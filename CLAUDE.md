...
| `obsidian_driver.py` | Stdlib-only driver that renders `data/` into the vault (each artist note → one hub wikilink in a two-tier tree: 13 color-coded top-level categories, second-order subcategory hubs beneath). Also folds in the streaming layer: a `rotation` class + evidence + by-year histogram per artist, a `Rotation` MOC carrying the merge's finding classes, and a `rotation` graph preset. Sibling to `validate.py`. |
