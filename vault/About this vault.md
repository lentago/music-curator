...
- Artists you **follow** on Spotify carry a `followed_at` and a **Followed:** line naming the song that triggered the follow when it was caught live. A follow can *seed* a new artist into the reservoir, so the `source-follow` preset shows the whole follow set at once — connected follows in their clusters and freshly seeded ones as loose nodes waiting to be tagged.
